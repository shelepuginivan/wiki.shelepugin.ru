...
import { ArchPkg } from '@components/packaging'

Гайд по настройке Arch Linux для игр.

Особенности:

-   Для запуска исполняемых файлов Windows (`.exe`) используется
    [wine](http://winehq.org/)
-   В качестве лаунчера используется [Lutris](https://lutris.net/)
-   Для систем с [Wayland](https://wayland.freedesktop.org/) используются
    [xwayland-satellite](https://github.com/Supreeeme/xwayland-satellite),
    [Cage](https://www.hjdskes.nl/projects/cage/) и
...
Чтобы включить `multilib`, откройте файл `/etc/pacman.conf`:
...
<ArchPkg pkg="lib32-pipewire" repo="multilib" /> — для воспроизведения аудио
...
В этом гайде рассмотрены xwayland-satellite, Cage и gamescope.

### xwayland-satellite

<Steps>

1.  Установите пакет <ArchPkg pkg="xwayland-satellite" />:
...
2.  Включите сервис `xwayland-satellite`:
    ...
    Если вы используете [Niri](https://github.com/YaLTeR/niri), пропустите
    этот шаг. Начиная с версии `25.08` в Niri есть [встроенная интеграция
    xwayland-satellite](https://yalter.github.io/niri/Xwayland.html).

    :::

3.  В настройках Lutris переменную окружения `DISPLAY` и установите значение
    `:0` — таким образом wine будет всегда использовать Xwayland для отображения.

    ![Установка переменной окружения DISPLAY в Lutris](./img/gaming-on-linux-xwayland-satellite-options.png)

</Steps>

После настройки любая игра, запускаемая с помощью wine, будет открываться с
помощью xwayland-satellite в отдельном окне.
...
5.  В качестве альтернативы можно создать вспомогательный скрипт и указать его
