...
Далее, см. [список важного ПО](/niri/usage/important-software), необходимого
для нормального использования рабочего стола, например, демон уведомлений и
порталы. 
Также ознакомьтесь с [обзором конфигурации](/niri/configuration/overview),
чтобы начать настраивать niri. Там вы найдете ссылки на другие страницы,
содержащие подробную документацию и примеры для всех опций. Наконец, страница
[Xwayland](/niri/usage/xwayland) объясняет, как запускать приложения для X11 в
niri.
...
[Kernel mode setting](https://wiki.archlinux.org/title/Kernel_mode_setting_(Русский)).
